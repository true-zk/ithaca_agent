...
from ithaca.agents.agent_types import EvaluationInput, EvaluationOutput


SYSTEM_PROMPT = """
You are a marketing evaluation agent.
You are responsible for evaluating the performance of a group of marketing plans.
After evaluation, you will give a score and summary of each marketing plan.
...
class EvaluationAgent:
    ...
    def get_prompt(self, input: EvaluationInput) -> str:
        ...
        Please:
        1. evaluate the performance of each marketing plan compared with the history marketing plans
        2. give a score and summary of each marketing plan (float from 1 to 10)
        3. output the evaluation results in the specified JSON format.
        """
        return prompt
    
    def _parse_response(self, response: Dict[str, Any]) -> Optional[EvaluationOutput]:
        """
        Parse the response from the evaluation agent.
        """
        try:
            output = EvaluationOutput.model_validate_json(response["structured_output"])
            return output
    ...
    def run(self, input: EvaluationInput) -> Dict[str, Any]:
        """
    ...
    async def run_async(self, input: EvaluationInput) -> Dict[str, Any]:
        """
        Run the evaluation agent asynchronously.
        """
